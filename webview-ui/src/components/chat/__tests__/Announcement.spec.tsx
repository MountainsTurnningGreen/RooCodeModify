	...
	useAppTranslation: () => ({
		t: (key: string, options?: { version: string }) => {
			if (key === "chat:announcement.title") {
				return `🎉 CryptContinue ${options?.version} Released`
			}
			if (key === "chat:announcement.description") {
				return `CryptContinue ${options?.version} brings powerful new features and improvements based on your feedback.`
			}
...
describe("Announcement", () => {
	...
	it("renders the announcement with the version number from package.json", () => {
		...
		expect(screen.getByText(`🎉 CryptContinue ${expectedVersion} Released`)).toBeInTheDocument()
		expect(
			screen.getByText(
				`CryptContinue ${expectedVersion} brings powerful new features and improvements based on your feedback.`,
			),
		).toBeInTheDocument()
	})
})
